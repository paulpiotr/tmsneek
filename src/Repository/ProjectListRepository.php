...
class ProjectListRepository extends ServiceEntityRepository
{
    ...
    public function findByHightPriority(?Request $request)
    {
        try {
            $qb = $this->createQueryBuilder('pl')
                ->innerJoin('App\Entity\TaskList', 'tl', 'WITH', 'tl.projectList=pl.id')
                ->andWhere('tl.priority>5')
                ->addOrderBy('pl.projectName')
            ;
            $paginator = $this->paginator($qb->getQuery(), (int) $request->get('page') > 0 ? (int) $request->get('page') : 1, (int) $request->get('limit') > 0 ? (int) $request->get('limit') : 5);
            return $paginator;
        } catch (\Exception $e) {
